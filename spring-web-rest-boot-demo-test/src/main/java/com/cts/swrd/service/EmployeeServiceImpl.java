...
public class EmployeeServiceImpl implements EmployeeService {
	...
	public Employee add(Employee emp) throws EmployeeException {
		if(emp!=null) {
			...
			if(empRepo.existsById(emp.getEmpId())) {
				throw new EmployeeException("There exists an employee wirh given ID");				
			}else if(empRepo.existsByMobileNumber(emp.getMobileNumber())) {
				throw new EmployeeException("There exists an employee with given number");
			}else if(empRepo.existsByEmail(emp.getEmail())) {
				throw new EmployeeException("There exists an employee with given mail");
	...
	public Employee save(Employee emp) throws EmployeeException {
		if(emp!=null) {
			...
			if(oldEmp==null) {
				throw new EmployeeException("There doesnot  exists an employee wirh given ID");				
			}else if(!oldEmp.getMobileNumber().equals(emp.getMobileNumber())&&empRepo.existsByMobileNumber(emp.getMobileNumber())) {
				throw new EmployeeException("There does not exists an empoyee with given number");
			}else if(!oldEmp.getEmail().equals(emp.getEmail())&&empRepo.existsByEmail(emp.getEmail())) {
				throw new EmployeeException("There does not exists an employee with given mail");
